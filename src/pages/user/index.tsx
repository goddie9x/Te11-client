...
const TViewUser = (props: RouteComponentProps<TMatchParamsTViewUser>) => {
  ...
  const handleSaveImage = ({ file, imageUrl }: onSaveProps) => {
    ...
    if (imageUrl) {
      dispatch(setLoading(true));
      fetch('https://te11api.herokuapp.com/user/profile/' + _id, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ image: imageUrl, tokenUser: localStorage.getItem('tokenUser') }),
      })
        .then((res) => {
          if (res.status >= 400) {
    ...
    if (file) {
      ...
      fetch('https://te11api.herokuapp.com/user/profile/avatar/' + _id, {
        method: 'PATCH',
        body: formData,
      })
        .then((res) => {
  ...
  return user?.deleted?<TTypography variant="h3" color="error" textalign="center" marginY={10}>
    ...
    <TGrid container position="relative">
      ...
      {user?._id ? (
        <TPost
          userId={user._id || 'nope'}
          titleRender={
            <TTypography textAlign="center" variant="h3" margintop={4} marginbottom={2}>
              {t('user_post', { user: user.fullName })}
            </TTypography>
          }
        />
      ) : null}
